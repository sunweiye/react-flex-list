...
var FlexList = function (_Component) {
    ...
    function FlexList(props) {
        ...
        _this._handleSearch = function (formData, changedFormFields) {
            ...
            if (!_utility2.default.isEmpty(_sorting) && (typeof _sorting === 'undefined' ? 'undefined' : _typeof(_sorting)) === 'object') {
            ...
            results = new _querySearch.QuerySearch().select(selection).from(_this.listData).where(queryWhere).sort(querySorting.join(',')).execute();
    ...
    _createClass(FlexList, [{
        ...
        value: function render() {
            var _props = this.props,
                ...
                containerProps = _objectWithoutProperties(_props, ['listData', 'listDataItemPreprocessor', 'renderItem', 'searchForm', 'searchTextFields', 'filtersFieldsMap', 'filtersVisibilityOnSearch', 'pageSize', 'paginationSettings', 'listContainerSettings', 'onListRender', 'beforeSearch', 'afterSearch', 'initialSearch']);
...
FlexList.propsTypes = {
    ...
    initialSearch: _propTypes2.default.object
};
